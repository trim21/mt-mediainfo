...
alter table
  thread
add
  column if not exists selected_size int8 not null default 0;


alter table
  thread
add
  column if not exists torrent_fetched_at timestamptz default null;


alter table
  job
  drop column if exists download_size;
...
create index if not exists thread_info_hash on thread (info_hash)
where
  info_hash != '';
...
where
  torrent_fetched_at is not null
  and selected_size > 0;
...
alter table
  job
add
  column if not exists dlspeed int8 not null default 0;


alter table
  job
add
  column if not exists eta int8 not null default -1;
...
create index if not exists idx_node_command_pending on node_command (node_id)
where
  executed_at is null;
