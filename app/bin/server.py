...
async def _fetch_progress_ctx(pool: asyncpg.Pool) -> dict[str, Any]:
    ...
    (
        ...
        where not (category = any($1)) and job.tid is null
        """,
            EXCLUDED_CATEGORY,
        ),
        pool.fetch(
...
def create_app() -> fastapi.FastAPI:
    ...
    async def _compute_today_stats() -> DailyStatsSnapshot:
        ...
        (
            ...
            pool.fetchrow(
                ...
                from thread
                where torrent_fetched_at >= $1 and torrent_fetched_at < $2
                  and selected_size > 0 and not (category = any($3))
                """,
                today,
                tomorrow,
                EXCLUDED_CATEGORY,
    ...
    async def threads_pending_mediainfo(
        ...
        return await _render_thread_list(
            ...
            count_params=[EXCLUDED_CATEGORY],
            page=page,
            show_progress=False,
    ...
    async def threads_pending_download(
        ...
        return await _render_thread_list(
            ...
            left join job on (job.tid = pending_download_threads.tid)
            where not (category = any($3)) and job.tid is null
            {order}
            limit $1 offset $2
            """,
            params=[EXCLUDED_CATEGORY],
    ...
    async def threads_downloading(render: Render, page: int = 1) -> HTMLResponse:
        return await _render_thread_list(
            ...
            from job
            join thread on (thread.tid = job.tid)
            where job.status = $1 and not (thread.category = any($2))
            """,
            rows_sql="""
            ...
            from job
            join thread on (thread.tid = job.tid)
            where job.status = $3 and not (thread.category = any($4))
            order by job.updated_at desc
            limit $1 offset $2
            """,
            params=[ItemStatus.DOWNLOADING, EXCLUDED_CATEGORY],
            page=page,
            show_progress=True,
            show_failed_reason=False,
        )
    ...
    async def threads_failed(render: Render, page: int = 1) -> HTMLResponse:
        return await _render_thread_list(
            ...
            limit $1 offset $2
            """,
            params=[ItemStatus.FAILED, EXCLUDED_CATEGORY],
            page=page,
            show_progress=False,
    ...
    async def threads_removed(render: Render, page: int = 1) -> HTMLResponse:
        return await _render_thread_list(
            ...
            limit $1 offset $2
            """,
            params=[ItemStatus.REMOVED_FROM_DOWNLOAD_CLIENT, EXCLUDED_CATEGORY],
            page=page,
            show_progress=False,
            show_failed_reason=True,
            show_reset=True,
    ...
    async def threads_bdmv(render: Render, page: Annotated[int, Query()] = 1) -> HTMLResponse:
        return await _render_thread_list(
            render,
            title="BDMV",
            count_sql="select count(1)::int from thread where type = 'bdmv' and not (category = any($1))",
            rows_sql="""
            select tid, category, size, selected_size, seeders, created_at from thread
            where type = 'bdmv' and not (category = any($3))
            order by created_at desc
            limit $1 offset $2
            """,
            params=[EXCLUDED_CATEGORY],
    ...
    async def threads_skipped(render: Render, page: Annotated[int, Query()] = 1) -> HTMLResponse:
        return await _render_thread_list(
            ...
            from job
            join thread on (thread.tid = job.tid)
            where job.status = 'skipped' and not (thread.category = any($1))
            """,
            rows_sql="""
            ...
            from job
            join thread on (thread.tid = job.tid)
            where job.status = 'skipped' and not (thread.category = any($3))
    ...
    async def nodes_page(render: Render) -> HTMLResponse:
        node_rows = await pool.fetch(
            "select id, last_seen, alias, version, status, debug_info from node order by id asc"
        )
