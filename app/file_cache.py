...
def encode_cached_files(files_data: list[tuple[int, str, int]]) -> bytes:
    return gzip.compress(orjson.dumps(files_data))


def decode_cached_files(data: bytes) -> list[tuple[int, str, int]]:
...
async def get_cached_files(
    ...
    await pool.execute(
        ...
        tid,
        encode_cached_files(files_data),
    )
    return files_data
