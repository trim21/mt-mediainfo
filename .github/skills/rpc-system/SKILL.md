...
The RPC system enables the web server to dispatch commands to specific nodes asynchronously via a PostgreSQL-backed command queue (`node_command` table).

## Architecture

1. **Server** enqueues commands via `POST /api/node/{node_id}/rpc` → inserts into `node_command` table
2. **Node** polls `node_command` for pending commands each iteration (before processing torrents)
3. **Node** executes the handler, writes `result`/`error` and `executed_at` back to the row
...
- `process_commands(db, node_id, handlers)` — Polls and executes pending commands (sync, called by node)
- `enqueue_command(pool, node_id, method, payload)` — Inserts a new command (async, called by server)

## Node-Side Handler Registration (`app/node.py`)
...
4. Implement handler in `app/node.py` `Node` class:
   ```python
   def __handle_cmd_my_method(self, payload: MyPayload) -> dict[str, str]:
       # ... do work ...
       return {"status": "ok"}
   ```
5. Register in `Node.__process_commands()` handlers dict
...
- `app/node.py` — Handler implementations, command polling in main loop
